...
class njc:
	...
	def __init__(self, bot):
		self.bot = bot
		self.channelstatus = 575846644185038848 #system-status
		self.channelID = 537717368688738363 #out-of-divisions
		self.channelID1 = 537717566588583956 #unknown-vehicles
		self.channelID2 = 537717700160520202 #unknown-branches
		self.channelID3 = 537718340106321960 #active-vehicles
		self.channelID4 = 646107374809251853 #autoscan-test
		self.channelID5 = 566968639476072449 #branch-checker
	...
	async def kirby(self):
		if lineNum:
		data.add_field = datetime.date.today().strftime("%Y-%m-%d %H:%M:%S").lstrip("0")
		data = self.get_bot_uptime()
		await discord.Embed ("<(module 'time' built-in)>")
                           "".format(passed, lines))
